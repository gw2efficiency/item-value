...
describe('itemValue', () => {
  ...
  it('skips the sell price for items that are inflated', () => {
    ...
    let item1 = {name: 'Warhorn', sell: {price: 10000000, last_known: 10, quantity: 10}, buy: {price: 1}}
    expect(itemValue(item1)).to.equal(1)

    // Whitelisted Item
    let item2 = {name: 'Fancy Skin', sell: {price: 10000000, last_known: 10, quantity: 10}, buy: {price: 1}}
    expect(itemValue(item2)).to.equal(10000000)

    // Buy price > 500g
    let item3 = {name: 'Warhorn', sell: {price: 100000000, last_known: 10, quantity: 10}, buy: {price: 5010000}}
    expect(itemValue(item3)).to.equal(100000000)

    // Supply > 1000
    let item4 = {name: 'Warhorn', sell: {price: 10000000, last_known: 10, quantity: 1001}, buy: {price: 1}}
    expect(itemValue(item4)).to.equal(10000000)

    // Sell Price < Buy Price * 2
    let item5 = {name: 'Warhorn', sell: {price: 100000, last_known: 10, quantity: 10}, buy: {price: 51000}}
    expect(itemValue(item5)).to.equal(100000)

    // Difference < 1g
    let item6 = {name: 'Warhorn', sell: {price: 10000, last_known: 10, quantity: 10}, buy: {price: 1}}
    expect(itemValue(item6)).to.equal(10000)
  })
})
